...
| CAD verification | Integrated proven | Exact-image committed verification passed after save/reopen | Visible verification UI and unsupported-check wording after final publish. |
| CAD generic STEP export | Source proven | Current bridge and renderer expose selected canonical generic STEP Part-21 export | Native picker/export/readback in the final desktop; no AP214/AP242/Inventor claim. |
...
| Java | Incomplete | Typed JDT session/provider lifecycle and fake protocol smoke exist; production intentionally uses `CreateUnprovisioned` | Provision a receipt-bound Eclipse JDT LS plus app-owned Java runtime and payload manifest, implement the production runtime authority, then mount real Java semantics. Enable after Arduino and before GCC. |
...
- Full frontend: 122 test files, 731 tests (fresh current-tree rerun at 2026-08-11 11:03 CDT).
...
- Focused Photon CAD frontend: 16 files, 158 tests.
- Photon CAD IndustrialProvider: 16/16.
- Photon CAD DesktopAdapter: 8/8.
- Photon CAD exact-image live-industrial bridge path: bearing + Spur Gear + preview + save/reopen + verification.
...
1. Manually prove visible CAD bearing creation and sealed preview first, then rotation, panel collapse/resize, save/reopen, verification and generic STEP export using the frozen mounted desktop.
