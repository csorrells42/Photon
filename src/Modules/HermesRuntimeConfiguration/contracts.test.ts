...
describe("Hermes runtime endpoint contracts", () => {
  it("accepts bounded HTTP endpoints and rejects credential-bearing or active-content URLs", () => {
    expect(
      normalizeRuntimeBaseUrl("http://host.docker.internal:11434/v1/"),
    ).toBe("http://host.docker.internal:11434/v1");
    expect(normalizeRuntimeBaseUrl("https://10.0.0.8:8080/v1")).toBe(
      "https://10.0.0.8:8080/v1",
    );
    expect(normalizeRuntimeBaseUrl("http://user:pass@127.0.0.1/v1")).toBeNull();
    expect(normalizeRuntimeBaseUrl("file:///C:/models")).toBeNull();
    expect(
      normalizeRuntimeBaseUrl("https://example.test/v1?token=secret"),
    ).toBeNull();
  });
  ...
  it("normalizes a renderer-safe endpoint inventory and drops malformed rows", () => {
          ...
          profiles: [
            {
              ...
              name: "Balanced",
              model: "gpt-oss:20b",
              overrides: { temperature: 0.4, top_k: 40 },
              is_active: true,
            },
          ],
        },
    ...
    expect(snapshot.current.apiMode).toBe("codex_responses");
    ...
    expect(snapshot.endpoints[0].profiles[0]).toMatchObject({
      id: "balanced",
      isActive: true,
      overrides: { temperature: 0.4, topK: 40 },
    });
    expect(JSON.stringify(snapshot)).not.toMatch(
      /api_key|password|token|secret/i,
    );
  });

  it("projects exact LM Studio native settings without inventing missing values", () => {
    const result = normalizeRuntimeEndpointValidation({
      ok: true,
      reachable: true,
      runtime_kind: "lm-studio",
      ...
      model_details: [
        {
          ...
          loaded_instances: [
            {
              id: "openai/gpt-oss-20b",
              config: { context_length: 65536, flash_attention: true },
            },
          ],
          capabilities: {
            reasoning: {
  ...
  it("serializes only explicit profile fields and never context", () => {
    const payload = runtimeProfilePayload({
      ...
      model: "gpt-oss",
      makeActive: true,
      overrides: { temperature: 0.5, topK: 40 },
    });
    expect(payload).toMatchObject({
      overrides: { temperature: 0.5, top_k: 40 },
      make_active: true,
    });
