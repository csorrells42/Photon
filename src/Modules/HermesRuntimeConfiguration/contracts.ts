...
export type HermesRuntimeModelDetail = {
  ...
  id: string
  loaded: boolean
  loadedInstances: Array<{ id: string; config: Record<string, boolean | number | string> }>
...
export type HermesRuntimeEndpointSnapshot = {
  current: { baseUrl: string; model: string; provider: string }
...
function boundedStrings(value: unknown, maximumItems = 2_000): string[] {
  ...
  return [...new Set(value.map((item) => boundedText(item, 512)).filter(Boolean))]
}
...
function normalizeProfileOverrides(value: unknown): HermesRuntimeProfileOverrides {
  const raw = record(value)
  const result: HermesRuntimeProfileOverrides = {}
  const mappings = [
    ['temperature', 'temperature', -100, 100], ['top_p', 'topP', 0, 1],
    ['frequency_penalty', 'frequencyPenalty', -100, 100], ['presence_penalty', 'presencePenalty', -100, 100],
    ['top_k', 'topK', 0, 1_000_000], ['min_p', 'minP', 0, 1], ['repeat_penalty', 'repeatPenalty', 0, 100],
    ['max_tokens', 'maxTokens', 1, 16_777_216], ['seed', 'seed', -2_147_483_648, 2_147_483_647],
  ] as const
...
function normalizeProfiles(value: unknown): HermesRuntimeProfile[] {
  ...
  return value.flatMap((candidate): HermesRuntimeProfile[] => {
    ...
    return [{ id, name, model, isActive: raw.is_active === true, overrides: normalizeProfileOverrides(raw.overrides) }]
  })
}
...
function normalizeModelDetails(value: unknown): HermesRuntimeModelDetail[] {
  ...
  return value.flatMap((candidate): HermesRuntimeModelDetail[] => {
    ...
    if (typeof capabilitiesRaw.vision === 'boolean') capabilities.vision = capabilitiesRaw.vision
    if (typeof capabilitiesRaw.trained_for_tool_use === 'boolean') capabilities.trainedForToolUse = capabilitiesRaw.trained_for_tool_use
    if (allowedOptions.length) capabilities.reasoning = { allowedOptions, ...(allowedOptions.includes(defaultReasoning) ? { default: defaultReasoning } : {}) }
    const loadedRaw = Array.isArray(raw.loaded_instances) && raw.loaded_instances.length <= 16 ? raw.loaded_instances : []
    const loadedInstances = loadedRaw.flatMap((instance): HermesRuntimeModelDetail['loadedInstances'] => {
      ...
      for (const [key, field] of Object.entries(configRaw).slice(0, 64)) {
        ...
        if (typeof field === 'boolean' || typeof field === 'string' && field.length <= 1_024 || typeof field === 'number' && Number.isFinite(field)) config[key] = field
      }
    ...
    const bitsPerWeight = optionalNumber(quantizationRaw.bits_per_weight, 0, 64)
    const detail: HermesRuntimeModelDetail = { id, type, loaded: raw.loaded === true, loadedInstances }
    const texts = [
      ['display_name', 'displayName', 256], ['publisher', 'publisher', 128], ['architecture', 'architecture', 128],
      ['format', 'format', 64], ['parameters', 'parameters', 64], ['selected_variant', 'selectedVariant', 512],
    ] as const
    ...
    const maxContextLength = optionalNumber(raw.max_context_length, 1, 16_777_216)
    const sizeBytes = optionalNumber(raw.size_bytes, 0, Number.MAX_SAFE_INTEGER)
    if (maxContextLength !== undefined) detail.maxContextLength = maxContextLength
    if (sizeBytes !== undefined) detail.sizeBytes = sizeBytes
    if (quantizationName || bitsPerWeight !== undefined) detail.quantization = { ...(quantizationName ? { name: quantizationName } : {}), ...(bitsPerWeight !== undefined ? { bitsPerWeight } : {}) }
...
export function assertRuntimeEndpointDraft(value: HermesRuntimeEndpointDraft): HermesRuntimeEndpointDraft {
  ...
  if (contextLength !== undefined && (!Number.isSafeInteger(contextLength) || contextLength < 1_024 || contextLength > 16_777_216)) {
    throw new Error('Context length must be between 1,024 and 16,777,216 tokens.')
  }
  return {
    ...(id ? { id } : {}), name, baseUrl, model,
...
export function normalizeRuntimeEndpointSnapshot(value: unknown): HermesRuntimeEndpointSnapshot {
  const raw = record(value)
  const currentRaw = record(raw.current)
  const endpointsRaw = Array.isArray(raw.endpoints) && raw.endpoints.length <= 256 ? raw.endpoints : []
  const endpoints = endpointsRaw.flatMap((candidate): HermesRuntimeEndpoint[] => {
    ...
    const source = endpoint.source === 'direct-config' ? 'direct-config' : 'providers'
    const contextLength = typeof endpoint.context_length === 'number' && Number.isSafeInteger(endpoint.context_length) && endpoint.context_length > 0
    ...
    return [{
      ...
      externalOverridesPresent: endpoint.external_overrides_present === true,
...
export function normalizeRuntimeEndpointValidation(value: unknown): HermesRuntimeEndpointValidation {
  const raw = record(value)
  return {
    ok: raw.ok === true,
    ...
    runtimeKind: raw.runtime_kind === 'lm-studio' ? 'lm-studio' : raw.runtime_kind === 'openai-compatible' ? 'openai-compatible' : 'unknown',
...
export function runtimeProfilePayload(draft: HermesRuntimeProfileDraft) {
  if (!identifierPattern.test(draft.id.trim()) || !boundedText(draft.name, 120) || !boundedText(draft.model, 512)) {
    throw new Error('Enter a valid profile ID, name, and exact model ID.')
  }
  const values: Record<string, unknown> = {}
  const mappings = [
    ['temperature', 'temperature'], ['topP', 'top_p'], ['frequencyPenalty', 'frequency_penalty'],
    ['presencePenalty', 'presence_penalty'], ['topK', 'top_k'], ['minP', 'min_p'],
    ['repeatPenalty', 'repeat_penalty'], ['maxTokens', 'max_tokens'], ['seed', 'seed'], ['stop', 'stop'],
  ] as const
  for (const [source, target] of mappings) {
    const value = draft.overrides[source]
    if (value !== undefined) values[target] = value
  }
  return { id: draft.id.trim(), name: draft.name.trim(), model: draft.model.trim(), overrides: values, make_active: draft.makeActive }
}
